            ...
            if (in_main_section) {
                ...
                if (line.at (0) == '_') {
                    ...
                    if (subroutines.count (subr_name) != 0 && subroutines[subr_name].initialized) {
                        ...
                        instructions[ins_idx++] = SBR_ENCODING;
                        instructions[ins_idx++] = (sbr_id & 0xFF);
                        instructions[ins_idx++] = (sbr_id >> 8);
                        subr_t subr = { sbr_id++, true };
                ...
                // The RSR command signifies the end of a subroutine
                if (!line.compare ("RSR")) {
                    if (!subr_encountered) {
                ...
                // Switch for the first operand. They can either be a variable ($), register (@), a label (#), subroutine (_), or address (&)
                switch (op1[0]) {
                    case '$': { // variable
